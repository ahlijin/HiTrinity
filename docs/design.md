...
> 全部在 Jetson Orin Nano (ARM64/aarch64, ROS2 Humble) 上编译通过。

| 工作空间 | 包数 | 类型 | 编译结果 |
|---------|------|------|---------|
| `jetson/` | 9 | Python + C++ (ament_cmake) | ✅ 全部通过 |
| `pi5/` | 4 | Python (ament_python) | ✅ 全部通过 |
| `pi3/` | 10 | Python + C++ (ament_cmake) | ✅ 全部通过（imu_calib Eigen 警告无影响） |

启动链路：
- **Jetson**: `trinity_jetson_start.sh` → `ros2 launch trinity_bringup jetson_bringup.launch.py`
- **Pi5**: `pi5_start.sh` → `ros2 launch pi5_bringup pi5_bringup.launch.py`
- **Pi3**: `pi3_start.sh` → `ros2 launch bringup pi3_bringup.launch.py`
- ROS_DOMAIN_ID=42 三机统一
